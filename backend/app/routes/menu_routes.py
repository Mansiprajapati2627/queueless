...
from app.utils.auth import get_db, get_current_admin_user   # ✅ both imports
...
def update_menu_item(item_id: int, item_update: MenuUpdate, db: Session = Depends(get_db), admin: User = Depends(get_current_admin_user)):
...
def delete_menu_item(item_id: int, db: Session = Depends(get_db), admin: User = Depends(get_current_admin_user)):
